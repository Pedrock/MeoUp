...
export class MeoCloud {
  ...
  constructor (credentials: { token: string, token_secret: string }) {
    const { token, token_secret } = credentials;
    this.config = { ...defaultConfig, oauth: { ...defaultConfig.oauth, token, token_secret } };
  }
  ...
  getOptions (requestMethod: 'get' | 'post' | 'put' | 'delete', endpoint: string | Array<string>, params: Object = {}, body?: Buffer | string) {
    const options = {
      ...
      uri: Array.isArray(endpoint) ? endpoint.join('') : endpoint,
      timeout: 0,
      form: undefined,
      body: undefined
    };
